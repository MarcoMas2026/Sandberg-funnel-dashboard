...
function TrendArrow({ up }: { up: boolean }) {
  ...
  return <Icon className="h-5 w-5 shrink-0 sm:h-6 sm:w-6" />;
}

function KpiIconBox({ children }: { children: React.ReactNode }) {
  return <span className="vantage-icon-box h-9 w-9 shrink-0 sm:h-11 sm:w-11">{children}</span>;
}
...
export function KpiCard({
  ...
  return (
    <div className="vantage-card flex h-full flex-col justify-between gap-3 p-4 sm:p-5">
      <div className="flex items-center gap-2.5">
        <KpiIconBox>{icon}</KpiIconBox>
        <p className="text-xs leading-tight text-[var(--vantage-text-muted)] sm:whitespace-nowrap sm:text-sm">{label}</p>
      </div>
...
export function PortfolioHealthCard({ value }: { value: number | null }) {
  ...
  const c = 2 * Math.PI * r;
  return (
    <div className="vantage-card flex h-full flex-col items-start justify-between gap-3 p-4 sm:flex-row sm:items-center sm:p-5">
      <div className="flex items-center gap-2.5">
        <KpiIconBox>
          <Folder className="h-5 w-5" />
        </KpiIconBox>
        <p className="text-sm text-[var(--vantage-text-muted)] sm:whitespace-nowrap">Portfolio Health</p>
      </div>
      <div className="self-center sm:self-auto">
        {value !== null ? (
          <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="h-[72px] w-[72px] shrink-0 sm:h-[100px] sm:w-[100px]">
  ...
  return (
    <Link href="/insights" className="vantage-pill block overflow-hidden">
      <div className="flex items-center gap-3 px-4 py-3 sm:px-5">
        ...
        <span className="shrink-0 text-xs text-[var(--vantage-text-muted)]">
